...
RELATION UPDATES: For records that already exist in discovery, emit an update/link on objectType notion.database_record with notion.dedupe.knownExternalId (never notion.knownExternalId alone), notion.relation.sourceRecordId, notion.relation.name, and notion.relation.targetRecordIds as a JSON array of exact related record IDs. Never authorize a mutation from a sentence that merely embeds IDs.

SAME-PLAN RELATIONS: When the source or related records are created earlier in this same immutable plan, do not invent future IDs and do not write \${action.output} strings. Authorize a typed dependency on property notion.relation as JSON: {"name":"Launch Checklist","source":{"recordActionId":"action-00N"},"targets":[{"recordActionId":"action-00M"}]} (baseActionId is equivalent). Exact discovered records use {"recordId":"<uuid>"}. Put every referenced create/reuse action in dependsOn. Planning authorizes only the typed dependency; apply resolves exact record IDs from that approved action's persisted result.
