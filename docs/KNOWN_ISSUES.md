...
- Recovery checkpoints are durable only after the agent returns a successful external action result. A process or transport failure in the narrow interval after a remote write but before its ID is returned is treated as ambiguous and must be reconciled through a fresh read-only inspection; Aurous will not replay it automatically.
